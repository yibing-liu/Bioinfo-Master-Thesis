...
for (j in chromosomeNumber){
  plotData_ratio = subset(data_combined, (data_combined$chr==j))[c(2, ((dim(data_combined)[2]-3)/2+4):(dim(data_combined)[2]))]
...
plot_logRratio = data.frame()
for (j in chromosomeNumber){
  plotData_ratio = subset(data_day6, (data_day6$chr==j))[c(2, ((dim(data_day6)[2]-3)/2+4):(dim(data_day6)[2]))]
  logR_ratio = melt(plotData_ratio, "start")
  logR_ratio$Chr = j
  plot_logRratio = rbind(plot_logRratio, logR_ratio)
}
